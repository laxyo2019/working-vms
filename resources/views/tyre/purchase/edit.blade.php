                              ...
                              <div class="col-sm-12 col-md-12 col-xl-12" id="mytable1">
                                 <div class='row'>
                                    <div class="col-md-3 col-xl-3 mt-2">
                                       ...
                                       <input id="po_no" name="po_number" class="form-control" readonly="true" value="{{old('po_number') ? old('po_no') : $orders->po_number }}">
                                       <input  name="id" class="form-control" readonly="true" type="hidden" value="{{$orders->id }}">
                                       @error('po_number')
                                    ...
                                    <div class="col-md-3 col-xl-3 mt-2">
                                       ...
                                       <input id="po_date" name="po_date" class="form-control datepicker" readonly="true" value="{{old('po_date') ? old('po_date') : $orders->po_date  }}">
                                       @error('po_date')
                                       <span class="invalid-feedback d-block" role="alert">
                                    ...
                                    <div class="col-md-3 col-xl-3 mt-2">
                                       ...
                                       <select id="vendor_code" required="true" name="vendor_code" class="form-control">
                                          ...
                                          <option value="{{$vendor->id}}"  @if($orders->vendor_code == $vendor->id) {{"selected"}}  @endif >{{$vendor->name}}</option>
                                          @endforeach	
                                       </select>
                                       @error('vendor_name')
                                       <span class="invalid-feedback d-block" role="alert">
                                 ...
                                 <div class="row">
                                    ...
                                    <div class="col-md-3 col-xl-3 mt-2">
                                       ...
                                       <select id="tyre_comp_name" name="tyre_comp_name"  class="form-control">
                                          ...
                                          {{-- <option value="{{$company->id}}" @if($orders->vendor_code == $vendor->id) {{"selected"}}  @endif>{{$company->comp_name}}</option> --}}
                                 ...
                                 <div class="row" style="" id="tabel">
                                                            ...
                                                            @foreach($ids as $id)
                                                               ...
                                                               @foreach($data1 as $data) 
                                                                  <tr data-id ="tr_{{$data['id']}}">
                                                                     ...
                                                                     <input type='hidden' value="{{$data['tyre_condition_id']}}" name = "tyre_condition_id[]" >
                                                                     <td style=" width:25%; ">{{$data['comp_name']}}</td>
                                                                     <td style=" width:25%;">{{$data['model_name']}}</td>
                                                                     <td style=" width:25%;">{{$data['tyre_condition']}}</td>
                                    ...
                                    <div class="col-md-4 col-xl-4 mt-2">
                                       ...
                                       <input readonly="true" id="total_qty" name='total_qty' class="form-control total" value="{{$orders->total_qty ? $orders->total_qty : ''}}">
                                       @error('total_qty')
                                       <span class="invalid-feedback d-block" role="alert">
                                    ...
                                    <div class="col-md-4 col-xl-4 mt-2">
                                       ...
                                       <input readonly="true"  name='grand_total' id="grand_total" class="form-control" value="{{$orders->grand_total ? $orders->grand_total : ''}}">
                                    </div>
                                    <div class="col-md-4 col-xl-4 mt-2">
                                       <label class="" for="Chasis No">Disc. Amount</label>
                                       <input readonly="true" id="disc_amt_sum" name='totl_disc_amt' class="form-control" value="{{$orders->disc_amt ? $orders->disc_amt : ''}}">
                                    </div>
                                    <div class="col-md-4 col-xl-4 mt-2">
                                       <label class="" for="Chasis No">IGST Amount</label>
                                       <input readonly="true" id="igst_amt_sum" name='totl_igst_amt' class="form-control  total" value="{{$orders->igst_amt ? $orders->igst_amt : ''}}">
                                    </div>
                                    <div class="col-md-4 col-xl-4 mt-2">
                                       <label class="" for="Chasis No">CGST Amount</label>
                                       <input  id="cgst_amt_sum" readonly="true" name='totl_cgst_amt' class="form-control  total" value="{{$orders->cgst_amt ? $orders->cgst_amt : ''}}">
                                    </div>
                                    <div class="col-md-4 col-xl-4 mt-2">
                                       <label class="" for="Chasis No">SGST Amount</label>
                                       <input readonly="true" id="sgst_amt_sum" name='totl_sgst_amt' class="form-control  total" value="{{$orders->sgst_amt ? $orders->sgst_amt : ''}}">
                                    </div>
                                    <div class="col-md-4 col-xl-4 mt-2">
                                       <label class="" for="Chasis No">Net Amount</label>
                                       <input readonly="true " id="net_amt_sum" name='totl_net_amt' class="form-control  total" value="{{$orders->net_amt ? $orders->net_amt : ''}}">
                                    </div>
                                 </div>
                                 <div class="row"  >
                                    <div class="col-md-12 col-xl-12 mt-2" >
                                       <label for="Chasis No">Remarks</label>
                                       <textarea id="email" name="remarks" class="form-control  " value="">{{old('remarks') ? old('remarks') : $orders->remarks }}</textarea>
                                       @error('remarks')
                                       <span class="invalid-feedback d-block" role="alert">
                                       <strong>{{ 'Please enter prepared by name in alphabets' }}</strong>
                                       </span>
...
<script type="text/javascript">
   ...
   $('#submit_form').click(function(){
       ...
       $.ajax({
           url: '{{route('update_purchase_save')}}',
           type: 'POST',
           headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
           data: $('#create_form1').serialize(),
           success: function(data) {
           window.location.replace("/tyre_purchase");
           }
       });
